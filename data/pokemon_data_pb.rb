# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: pogoprotos/data/pokemon_data.proto

require 'google/protobuf'

require 'pogoprotos/data/pokemon_display_pb'
require 'pogoprotos/enums/pokemon_id_pb'
require 'pogoprotos/enums/pokemon_move_pb'
require 'pogoprotos/inventory/item/item_id_pb'
Google::Protobuf::DescriptorPool.generated_pool.build do
  add_message "POGOProtos.Data.PokemonData" do
    optional :id, :fixed64, 1
    optional :pokemon_id, :enum, 2, "POGOProtos.Enums.PokemonId"
    optional :cp, :int32, 3
    optional :stamina, :int32, 4
    optional :stamina_max, :int32, 5
    optional :move_1, :enum, 6, "POGOProtos.Enums.PokemonMove"
    optional :move_2, :enum, 7, "POGOProtos.Enums.PokemonMove"
    optional :deployed_fort_id, :string, 8
    optional :owner_name, :string, 9
    optional :is_egg, :bool, 10
    optional :egg_km_walked_target, :double, 11
    optional :egg_km_walked_start, :double, 12
    optional :origin, :int32, 14
    optional :height_m, :float, 15
    optional :weight_kg, :float, 16
    optional :individual_attack, :int32, 17
    optional :individual_defense, :int32, 18
    optional :individual_stamina, :int32, 19
    optional :cp_multiplier, :float, 20
    optional :pokeball, :enum, 21, "POGOProtos.Inventory.Item.ItemId"
    optional :captured_cell_id, :uint64, 22
    optional :battles_attacked, :int32, 23
    optional :battles_defended, :int32, 24
    optional :egg_incubator_id, :string, 25
    optional :creation_time_ms, :uint64, 26
    optional :num_upgrades, :int32, 27
    optional :additional_cp_multiplier, :float, 28
    optional :favorite, :int32, 29
    optional :nickname, :string, 30
    optional :from_fort, :int32, 31
    optional :buddy_candy_awarded, :int32, 32
    optional :buddy_total_km_walked, :float, 33
    optional :display_pokemon_id, :int32, 34
    optional :display_cp, :int32, 35
    optional :pokemon_display, :message, 36, "POGOProtos.Data.PokemonDisplay"
  end
end

module POGOProtos
  module Data
    PokemonData = Google::Protobuf::DescriptorPool.generated_pool.lookup("POGOProtos.Data.PokemonData").msgclass
  end
end
